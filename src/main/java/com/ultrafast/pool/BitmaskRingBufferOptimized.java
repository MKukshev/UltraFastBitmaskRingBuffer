...
/**
 * <b>BitmaskRingBufferOptimized</b> — высокопроизводительный пул объектов с использованием VarHandle.
 * Оптимизированная версия для максимальной производительности в многопоточной среде.
 * Использует VarHandle вместо AtomicReferenceArray для прямого доступа к массиву объектов.
 *
 * <p>Основные оптимизации:
 * <ul>
 *   <li>VarHandle для прямого доступа к массиву объектов</li>
 *   <li>Меньше абстракций и лучшая inlining-оптимизация JIT</li>
 *   <li>Более эффективные CAS-операции</li>
 * </ul>
 *
 * @param <T> Тип объектов в пуле (например, Task)
 */
public class BitmaskRingBufferOptimized<T> {
    /** Размер буфера (степень 2 для оптимизации побитовых операций) */
    private final int capacity;
    /** Маска для быстрого вычисления индекса (capacity - 1) */
    private final int mask;
    /** Массив объектов пула (используется с VarHandle) */
    ...
    /** VarHandle для атомарного доступа к элементам массива objects */
    private static final VarHandle OBJECTS_HANDLE = MethodHandles.arrayElementVarHandle(Object[].class);
    ...
    public BitmaskRingBufferOptimized(int capacity, Supplier<T> objectFactory) {
    ...
    public T getFreeObject(long maxWaitNanos) {
        ...
        while (true) {
            ...
            if (head.compareAndSet(currentHead, currentHead + 1)) {
                ...
                T object = (T) OBJECTS_HANDLE.getVolatile(objects, index);
                if (object == null) {
                    object = objectFactory.get();
                    OBJECTS_HANDLE.setVolatile(objects, index, object);
                }
    ...
    public List<T> getOccupiedObjects() {
        ...
        for (int i = 0; i < capacity; i++) {
            if (getBit(availabilityMask, i)) {
                T object = (T) OBJECTS_HANDLE.getVolatile(objects, i);
                if (object != null) {
                    result.add(object);
                }
            }
    ...
    private void initializeObjects() {
        for (int i = 0; i < capacity; i++) {
            ...
            OBJECTS_HANDLE.setVolatile(objects, i, object);
        }
    }
    ...
    private int findObjectIndex(T object) {
        for (int i = 0; i < capacity; i++) {
            if (OBJECTS_HANDLE.getVolatile(objects, i) == object) {
                return i;
            }
        }
        return -1;
