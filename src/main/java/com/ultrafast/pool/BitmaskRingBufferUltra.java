...
/**
 * Ultra-fast concurrent object pool combining off-heap memory and bit tricks for maximum performance.
 * 
 ...
 * 
 * Key optimizations:
 * - Off-heap bitmasks avoid GC pauses and false sharing
 ...
 */
public class BitmaskRingBufferUltra<T> {
    ...
    /**
     * Creates a new ultra-optimized ring buffer pool combining off-heap and bit tricks.
     * 
     * @param capacity Maximum number of objects in the pool
     * @param objectFactory Factory to create new objects
     */
    @SuppressWarnings("unchecked")
    public BitmaskRingBufferUltra(int capacity, ObjectFactory<T> objectFactory) {
        if (capacity <= 0) {
    ...
    public boolean markForUpdate(T object) {
        ...
        for (int i = 0; i < capacity; i++) {
            if (objects[i] == object) {
                // setBit(updateMaskAddr, i, true); // Удалено
    ...
    public List<T> getObjectsForUpdate() {
        ...
        for (int i = 0; i < capacity; i++) {
            // if (isBitSet(updateMaskAddr, i)) { // Удалено
    ...
    public boolean isMarkedForUpdate(int index) {
        ...
        // return isBitSet(updateMaskAddr, index); // Удалено
        return false;
    }
